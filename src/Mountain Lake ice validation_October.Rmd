...
test = rs_clean %>% 
  ...
  geom_vline(data = ref_clean %>% filter(water_year >= t1, water_year <= t2, lakename == "Albion"), aes(xintercept = sdoy, color =     event), lwd = 1) +
...
merged_pdates %>% 
  ...
  theme_bw() +
  labs(x = "Days since 10/1 from MODIS",
       y = "Days since 10/1 from in situ",
       color = "Lake name" #,
       ) #pch = "Lake name"
